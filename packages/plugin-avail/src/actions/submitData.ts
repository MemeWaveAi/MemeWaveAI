...
export default {
    ...
    handler: async (
        ...
        try {
            ...
            const submitDataInfo = await api.tx.dataAvailability.submitData(data).paymentInfo(keyring)
          ...
          `)
            ...
            const txResult = await new Promise<ISubmittableResult>((res) => {
                api.tx.dataAvailability.submitData(data).signAndSend(keyring, options, (result: ISubmittableResult) => {
            ...
            if (callback) {
                callback({
                    text:
                        `Data submitted successfully! tx hash: ${txResult.txHash as H256} Block Hash: ${txResult.status.asFinalized as H256} `,
                    content: {},
                });
            }
