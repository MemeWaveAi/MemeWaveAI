...
export default {
    ...
    handler: async (
        ...
        try {
            ...
            const oldBalance: any = await api.query.system.account(content.recipient)
            elizaLogger.log(`Balance before the transfer call: ${oldBalance["data"]["free"].toHuman()}`)
            ...
            const txResult = await new Promise<ISubmittableResult>((res) => {
                ...
                .signAndSend(keyring, options, (result: ISubmittableResult) => {
                    elizaLogger.log(`Tx status: ${result.status}`)
            ...
            const newBalance: any = await api.query.system.account(content.recipient)
            elizaLogger.log(`Balance after the transfer call: ${newBalance["data"]["free"].toHuman()}`)
            ...
            if (callback) {
                callback({
                    text:
                        `Transfer completed successfully! tx hash: ${txResult.txHash as H256} Block Hash: ${txResult.status.asFinalized as H256} `,
                    content: {},
                });
            }
